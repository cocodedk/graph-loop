...
requirement:
  goal: 'A builder call that ends `limit`, `auth` or `capacity` after it had already spent figures charges
    the card no rebuild round: the card goes back to `todo` in the same worktree with the `rebuild_round`
    it already had, and the `rebuild_queued` event records the round as not charged. A card at round 2
    that meets a limit is still `todo` at round 2, not `rejected`. A call that ended `crash` still charges
    its round.'
  done_when: 'The gate passes. With the `test_loop` rig: for each of `limit`, `auth` and `capacity`, a
    build call that returns that kind with `cost=1.2, tokens=500` leaves the card `todo`, `rebuild_round`
    unset or 0, `rebuild_from` equal to the outcome''s worktree, and the last `rebuild_queued` event for
    the card carrying `charged` exactly `False`. A card starting at `rebuild_round: 2` that meets such
    a limit is still `("todo", 2)`. A build call that returns `crash` with the same figures leaves `rebuild_round`
    1. graph/tests/test_loop.py, graph/tests/test_loop_rebuild.py, graph/tests/test_backlog_rounds.py,
    graph/tests/test_live_accounts.py and graph/tests/test_keep_combined_owner.py stay green. The gate
    does not prove anything about cooldowns, about which provider is called next, or about how a limit
    is classified by `triage_signatures`.'
  sources:
  - docs/rfc/stalls-brief.md:31
  - docs/rfc/stalls-brief.md:32
contract_seen: 6ea7b77e8c6b5fac
accepted_criteria:
  goal: 'A builder call that ends `limit`, `auth` or `capacity` after it had already spent figures charges
    the card no rebuild round: the card goes back to `todo` in the same worktree with the `rebuild_round`
    it already had, and the `rebuild_queued` event records the round as not charged. A card at round 2
  ...
  gate: "set -e -o pipefail\ntimeout 600 python3 - <<'PY'\nimport os, pathlib, sys\nroot = pathlib.Path(os.getcwd()).resolve()\n\
    ...
    \ limit charges the card a round\"\n\n\ndef paid(kind):\n    \"\"\"A call that spent figures and then\
    \ hit the provider's wall.\"\"\"\n    return Fakes(build=[Outcome(kind, text=f\"{kind} reached mid-call\"\
    , cost=1.2, tokens=500),\n                        Outcome(kind, text=\"never called\")])\n\n\nfor\
    \ kind in (\"limit\", \"auth\", \"capacity\"):\n    loop, book, space = loop_for(task(), paid(kind))\n\
    \    out = loop.run_task(book.task(\"T1\"))\n    row = book.task(\"T1\")\n    assert row[\"status\"\
    ] == \"todo\", f\"{RED}: {kind} left the card {row['status']}\"\n    assert int(row.get(\"rebuild_round\"\
    ) or 0) == 0, \\\n        f\"{RED}: {kind} charged round {row.get('rebuild_round')}\"\n    assert\
    \ row.get(\"rebuild_from\") == out.worktree, \\\n        f\"{RED}: {kind} lost the worktree: {row.get('rebuild_from')!r}\"\
    \n    queued = [one for one in space.events()\n              if one.get(\"kind\") == \"rebuild_queued\"\
    \ and one.get(\"task\") == \"T1\"]\n    assert queued and queued[-1].get(\"charged\") is False, \\\
    \n        f\"{RED}: the {kind} round was recorded as charged: {queued}\"\n\n# The rebuild state it\
    \ already had survives, and a limit at round 2 does not\n# reject it: that is the ending the campaign\
    \ actually recorded.\nloop, book, _ = loop_for(task(rebuild_round=2), paid(\"limit\"))\nloop.run_task(book.task(\"\
    T1\"))\nrow = book.task(\"T1\")\nassert (row[\"status\"], int(row.get(\"rebuild_round\") or 0)) ==\
    ...
    loop.run_task(book.task(\"T1\"))\nrow = book.task(\"T1\")\nassert int(row.get(\"rebuild_round\") or\
  ...
  - graph/lib/loop_steps.py
  - graph/tests/test_loop_faults.py
rebuild_from: /var/tmp/graph-trees/graph-d9znui7c/task-a-provider-limit-charges-no-round
session: 926eba80-7107-49fc-93df-96b04a4c42e8
...
Dropped: delivered outside the campaign by commits 3a1511f and 611949f on fix/blender, which pass this card's gate; the loop's own builder had stopped BLOCKED over a denied command.
