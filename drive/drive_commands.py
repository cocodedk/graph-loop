...
`drive-goal.py` keeps `run` — the loop that works the backlog — and the command
...
CLAUDE_BIN = os.environ.get("DRIVE_CLAUDE", "claude")
CODEX_BIN = os.environ.get("DRIVE_CODEX", "codex")
...
def command_init(args) -> int:
    ...
    if kept.path != book.path:
        print(f"this campaign already existed: it still points at {kept.path}, "
              f"not at {book.path}")
    print("nothing runs until: drive-goal.py approve")
    return 0
