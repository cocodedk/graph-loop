...
own configuration directory, and name it in DRIVE_ACCOUNTS.
"""
...
def table() -> tuple[tuple[str, str | None], ...]:
    """The accounts, from DRIVE_ACCOUNTS when it is set, else the one default.

    DRIVE_ACCOUNTS is `name=configdir` pairs separated by commas, a bare name
    meaning the default configuration: `work,second=/etc/claude/second`.
    """
    raw = os.environ.get("DRIVE_ACCOUNTS", "").strip()
    if not raw:
        return _DEFAULT
    out = []
