...
def codex_text(binary: str, prompt: str, *, model: str, effort: str,
               cwd: str = "", timeout: int = 1800) -> Outcome:
    """One codex call, read-only, answering with whatever text it wrote.
    ...
    argv.append("-")
    try:
        done = _run(argv, prompt, None, timeout)
