...
One list per job, tried in order until one answers. DRIVE_BUILDERS,
DRIVE_REVIEWERS and DRIVE_CLAUDE_REVIEWERS override them without touching this
...
def builders() -> tuple[str, ...]:
    return _listed("DRIVE_BUILDERS", _BUILDERS)


def reviewers() -> tuple[str, ...]:
    return _listed("DRIVE_REVIEWERS", _REVIEWERS)


def claude_reviewers() -> tuple[str, ...]:
    """The claude rungs of the review belt: never the builder list."""
    return _listed("DRIVE_CLAUDE_REVIEWERS", _CLAUDE_REVIEWERS)
