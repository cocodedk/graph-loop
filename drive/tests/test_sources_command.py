...
class SourceEscapeTest(unittest.TestCase):
    def setUp(self):
        ...
        patch = unittest.mock.patch.object(drive_commands, "REPO", str(self.repo))
        patch.start(); self.addCleanup(patch.stop)

    def test_a_dotdot_path_is_refused(self):
        with self.assertRaises(SystemExit):
            drive_commands.declare_sources(Space(), [f"../{self.outside.name}/b.md"])

    def test_a_symlink_out_of_the_repo_is_refused(self):
        (self.repo / "sneaky").symlink_to(self.outside)
        with self.assertRaises(SystemExit):
            drive_commands.declare_sources(Space(), ["sneaky/b.md"])

    def test_a_good_path_is_stored_normalized_and_recorded(self):
        space = Space()
        good = drive_commands.declare_sources(space, ["spec/../spec/a.md"])
