...
def run_supervisor(case: unittest.TestCase, run_exit: int,
    ...
    (stub / "python3").write_text(
        "#!/bin/bash\n"
        'case "$*" in\n'
        f'  *"drive-goal.py run"*) exit {run_exit} ;;\n'
        f'  *"drive-goal.py plan"*) exit {plan_exit} ;;\n'
        '  *"drive-goal.py report"*) echo report ;;\n'
    ...
    env = dict(os.environ, PATH=f"{stub}:{os.environ['PATH']}",
               DRIVE_CAMPAIGN=str(camp))
    done = subprocess.run(["bash", str(DRIVE / "supervisor.sh")], env=env, check=False,
...
class UndeliveredFatalTest(unittest.TestCase):
    ...
    def test_a_stand_down_nobody_was_told_about_stays_on_the_board(self):
        ...
        check = subprocess.run(["bash", str(DRIVE / "watch.sh"), "--check"], check=False,
                               env=dict(os.environ, DRIVE_CAMPAIGN=str(camp)),
...
class TwoStandDownsTest(unittest.TestCase):
    ...
    def test_delivering_the_older_notice_leaves_the_newer_one_standing(self):
        ...
        board = subprocess.run(["bash", str(DRIVE / "watch.sh"), "--check"], check=False,
                               env=dict(os.environ, DRIVE_CAMPAIGN=str(camp)),
