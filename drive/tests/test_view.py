...
class LoopProcessTest(unittest.TestCase):
    ...
    def test_the_driver_is_drive_goal_run(self):
        self.assertEqual("driver", is_loop_process(["2", "00:05", "python3", "/x/drive-goal.py", "run", "--idle-seconds", "300"]))
        self.assertEqual("", is_loop_process(["3", "00:01", "python3", "/x/drive-goal.py", "report"]))
...
class DriverStartTest(unittest.TestCase):
    def test_the_report_snapshot_is_not_the_driver(self):
        rows = [["4", "9", "python3", "/x/drive-goal.py", "report"]]
        self.assertIsNone(_driver_start(rows, [{"kind": "driver_started", "pid": 4}]))
        rows.append(["41", "9", "python3", "/x/drive-goal.py", "run"])
    ...
    def test_the_start_is_this_campaigns_driver_not_the_hosts_first(self):
        rows = [["33", "9000", "python3", "/y/drive-goal.py", "run"],       # another campaign's driver, old
                ["41", "9", "python3", "/x/drive-goal.py", "run"]]           # ours, just started
        started = _driver_start(rows, [{"kind": "driver_started", "pid": 41}])
...
class RolloutTest(unittest.TestCase):
    def test_a_driver_that_predates_the_flag_is_switched_by_the_stop_flag(self):
        ...
        self.assertIn("start `bash drive/supervisor.sh`", line)
    ...
    def test_the_driver_pid_is_this_campaigns_announced_driver_not_the_hosts_first(self):
        rows = [["4", "9", "python3", "/x/drive-goal.py", "report"],
                ["33", "9", "python3", "/y/drive-goal.py", "run"],                 # another campaign's driver, first on the host
                ["41", "9", "python3", "/x/drive-goal.py", "run", "--idle-seconds", "300"],
    ...
    def test_a_supervisor_counts_only_when_it_owns_the_driver_and_knows_the_restart_exit(self):
        ...
        child = ["41", "7", "9", "python3", "/x/drive-goal.py", "run"]
        orphan = ["41", "1", "9", "python3", "/x/drive-goal.py", "run"]
        ...
        report = ["42", "7", "2", "python3", "/x/drive-goal.py", "report"]             # the supervisor's own snapshot
        self.assertFalse(supervisor_owns_driver(42, [sup, report]))                      # a child, but not the driver
        other = ["99", "7", "9", "python3", "/y/drive-goal.py", "run"]                  # another campaign's pair
    ...
    def test_without_a_supervisor_the_advice_is_to_start_one(self):
        ...
        self.assertIn("then remove the stop flag and start `bash drive/supervisor.sh`", line)
