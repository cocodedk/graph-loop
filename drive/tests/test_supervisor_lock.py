...
def _stub_env(camp: pathlib.Path, stub: pathlib.Path) -> dict:
    ...
    (stub / "python3").write_text(
        "#!/bin/bash\n"
        'case "$*" in\n'
        '  *"drive-goal.py run"*) sleep 2; exit 0 ;;\n'
        '  *"drive-goal.py report"*) echo report ;;\n'
        f'  *) exec "{real}" "$@" ;;\n'
        "esac\n")
    (stub / "python3").chmod(0o755)
    return dict(os.environ, PATH=f"{stub}:{os.environ['PATH']}", DRIVE_CAMPAIGN=str(camp))
...
class LockContentionTest(unittest.TestCase):
    def test_second_supervisors_lock_message_stays_off_the_log(self):
        ...
        first = subprocess.Popen(["bash", str(DRIVE / "supervisor.sh")], env=env,
        ...
        started = time.time()
        second = subprocess.run(["bash", str(DRIVE / "supervisor.sh")], env=env, check=False,
