...
from drive_commands import replan_pending
...
class PlannerCallTest(unittest.TestCase):
    def test_the_planner_has_no_tools_and_its_own_shorter_timeout(self):
        import turn as drive_commands
        seen = {}
        def fake(binary, prompt, **kw):
            seen.update(kw); return Outcome("ok", text="")
        original = drive_commands.claude
        drive_commands.claude = fake
        try:
            drive_commands.plan_with_claude("p")
        finally:
            drive_commands.claude = original
...
class RealReviewCwdTest(unittest.TestCase):
    ...
    def test_the_codex_call_changes_into_the_given_worktree_read_only(self):
        ...
        with unittest.mock.patch.object(review.resources, "belt", return_value=belt), \
            ...
            out = drive_commands._real_review("judge this", cwd="/worktrees/T1", effort="high")
...
class RealReviewLedgerTest(unittest.TestCase):
    ...
    def test_a_review_leaves_one_ledger_row_under_its_account_and_task(self):
        ...
        with unittest.mock.patch.object(drive_commands, "codex", fake_codex):
            drive_commands._real_review("judge this", cwd="/worktrees/T1", effort="high",
