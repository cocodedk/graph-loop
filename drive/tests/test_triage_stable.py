...
EXPECTED_TESTS = 9
CHECKOUT = "/tmp/drive-tests-9zk/drive-a1b2/task-T1"


class CheckoutTest(unittest.TestCase):
    def test_only_the_checkout_prefix_goes_and_the_path_under_it_stays(self):
        self.assertEqual("<worktree>/pkg/drive-cache/task-one/a.py:12 it said one",
                         stable(f"{CHECKOUT}/pkg/drive-cache/task-one/a.py:12 it said one"))

    def test_two_relative_directories_are_two_failures(self):
        """`pkg/drive-cache/task-one` is a directory somebody wrote, not a
        checkout the loop cut: a greedy parent match ate both and left one
        failure where there were two."""
        self.assertNotEqual(stable(f"{CHECKOUT}/pkg/drive-cache/task-one/a.py failed"),
                            stable(f"{CHECKOUT}/pkg/drive-cache/task-two/a.py failed"))

    def test_the_checkout_is_blanked_wherever_it_was_cut(self):
        self.assertEqual(stable(f"{CHECKOUT}/a.py failed"),
                         stable("/var/tmp/drive-99/task-T1/a.py failed"))
    ...
    def test_a_path_written_as_a_uri_is_still_a_path(self):
        ...
        self.assertEqual("file://<worktree>/pkg/a.py:12 failed",
                         stable("file:///tmp/drive-a/task-T1/pkg/a.py:12 failed"))
        self.assertEqual(stable("file:///tmp/drive-a/task-T1/a.py:12 failed"),
                         stable("file:///tmp/drive-b/task-T1/a.py:12 failed"))
    ...
    def test_a_doubled_slash_is_not_where_a_path_begins(self):
        ...
        one made `pkg//drive-cache/task-one` a checkout, and two directories
    ...
    def test_a_uri_keeps_two_lines_of_one_file_apart(self):
        self.assertNotEqual(stable("file:///tmp/drive-a/task-T1/a.py:12 failed"),
                            stable("file:///tmp/drive-a/task-T1/a.py:55 failed"))
