...
once at driver start (`command_run`, drive-goal.py) before the turn ever got
...
_spec = importlib.util.spec_from_file_location("drive_goal_flags", HERE / "drive-goal.py")
...
class StopFlagAtDriverStartTest(unittest.TestCase):
    def test_a_flag_placed_before_the_turn_starts_no_task_and_survives_it(self):
        """Mirrors driver start: a person's `drive-goal.py stop` writes
        ...
        with unittest.mock.patch.object(Keeper, "pending", lambda self: []), \
             ...
             unittest.mock.patch.object(drive_goal, "run_lanes", fake_run_lanes):
