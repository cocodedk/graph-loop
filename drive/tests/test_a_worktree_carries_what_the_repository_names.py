...
what to carry from `DRIVE_PROVISION_COPY` and `DRIVE_PROVISION_LINK` now, and a
...
class ProvisionTest(unittest.TestCase):
    def test_nothing_named_means_a_plain_checkout(self):
        repo, tree = trees()
        with mock.patch.dict(os.environ, {"DRIVE_PROVISION_COPY": "",
                                          "DRIVE_PROVISION_LINK": ""}):
    ...
    def test_a_named_folder_is_linked_not_copied(self):
        repo, tree = trees()
        with mock.patch.dict(os.environ, {"DRIVE_PROVISION_LINK": "interpreter"}):
            provision(str(tree), str(repo))
