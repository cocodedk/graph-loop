...
records nothing; this file proves that `drive_commands._real_review` — what
...
def fake_codex(binary, prompt, *, cwd="", effort="", attempt=None):
    ...
    `test_drive_commands.RealReviewLedgerTest` does — one accepted call,
...
class RealReviewLedgerTest(unittest.TestCase):
    def test_both_reviews_of_a_task_land_in_the_campaign_ledger(self):
        ...
        loop = Loop(repo=root, backlog=book, space=space, build=fakes.builder,
                    review=drive_commands._real_review)
