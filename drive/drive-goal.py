#!/usr/bin/env python3
"""drive-goal — work a backlog to done, and say so when it cannot.

    drive-goal.py init --backlog <file> [--goal "..."] [--branch <name>] [--source <path>]...
    drive-goal.py approve
    drive-goal.py plan [--rounds N]
    drive-goal.py run [--lanes 3] [--max-tasks N] [--dry-run]
    drive-goal.py status
    drive-goal.py report
    drive-goal.py doctor
    drive-goal.py stop [--now]
...
The campaign lives in `drive/campaign/` unless `--workspace` says
...
# which repository and campaign — override with DRIVE_REPO / DRIVE_CAMPAIGN /
# DRIVE_BACKLOG / DRIVE_BRANCH to point the loop at other work.
...
def command_run(args) -> int:
    ...
    if not (space.root / "approved").exists():
        raise SystemExit("not approved — run `drive-goal.py approve` first")
