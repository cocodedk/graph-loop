...
TRIAGE names why a drive-loop run ended badly and sends that cause to the
...
`lib/triage.py:triage_pending` after `drive-goal.py` reconciles and pushes pending
...
The full drive suite ran 636 tests in this worktree: 635 passed. The one stable
failure is the hardcoded-path driver defect recorded below, outside TRIAGE.

## Impediments — integration ledger

1. **Done:** `drive-goal.py` — the reconcile and push-if-behind boundary the call
