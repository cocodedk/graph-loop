...
passes, when nothing can move, or when you say so.
    ...
    python3 drive/drive-goal.py init    --backlog "$DRIVE_REPO/vault" --source spec/brief.md
    python3 drive/drive-goal.py approve --revision 1
    python3 drive/drive-goal.py run
    python3 drive/drive-goal.py status
    python3 drive/drive-goal.py stop        # after the running tasks
    python3 drive/drive-goal.py stop --now  # kill the recorded groups, keep the worktrees
