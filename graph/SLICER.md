...
- **The graph loop consumes atoms and builds them.** It stops authoring work
  entirely; the commander stops hand-writing cards.
- **The graph loop's contract review is the slicer's gate.** It already
...
to edit `graph-goal.py`, `lib/turn.py` or another existing driver file.
...
`goal → spec → molecule → atom → unchanged graph → gate`.

## Plan

The slicer has one job: keep the graph loop supplied with buildable work from
approved sources. It plans work; the graph loop still proves, reviews, builds,
...
6. Send every new atom through the graph loop unchanged: naming law, contract
...
7. Add the smallest tests for the new transition, assert their count, run the
   graph suite and the linter on touched code. An independent reviewer checks each
...
slicer until the graph stops growing and builds nothing; `graph-goal.py run`
...
Every generated CODE gate is stored with `gate_reviewed_first: true`; the graph
