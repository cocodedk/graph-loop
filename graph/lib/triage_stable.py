...
# The checkout the round ran in. `Worktree` cuts `mkdtemp(prefix="graph-")`
...
# `pkg/graph-cache/task-one` is a directory somebody wrote, and a greedy
...
# let a doubled slash start a path, so `pkg//graph-cache/task-one` became a
# checkout and two directories somebody wrote came out as one failure.
START = r"(?:(?<![\w./-])|(?<=file://))"
CHECKOUT = re.compile(START + r"(?:/[\w.-]+)*?/graph-[^/\s]+/task-[^/\s]+")
