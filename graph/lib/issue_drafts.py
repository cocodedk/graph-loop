...
def _evidence(space, rows: list[dict], card: dict) -> tuple[str, str]:
    ...
    cause = (next((row for row in reversed(causes) if row.get("step")), {})
             if last.get("kind") == "rejected" else last)
    step = str(last.get("step") or timed.get("step") or cause.get("step")
    ...
    if step == "keep" and timed.get("clash"):
        step = "combined_gate"
    why = str(last.get("why") or card.get("refused_why") or "No ending text was recorded.")
    ...
    if name:
        ...
        why = _read(artifact.get("path"), space.root) or why
    return step, why
