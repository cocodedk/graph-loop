...
The destination is configurable (`init --branch`, `GRAPH_BRANCH`) so the loop
...
def resolve_argv(branch: str) -> list[str]:
    ...
    the one asked for, so a branch `campaign/graph/x` would answer for a
    `campaign/graph` that is not there.
    """
