...
the campaign it started you for, and that campaign's `GRAPH_CAMPAIGN` and
`GRAPH_BRANCH` are set in your session. One campaign's stand-in is never
...
remote-controlled so the owner can watch and graph from their phone. WATCHER
normally reads every red-board message from the graph campaign's supervisor
...
Never touch the driver's own files (`graph-goal.py`, `lib/`) while it is
...
A `Stop` hook (`graph/heartbeat.sh`) already keeps
...
- `bash graph/watch.sh --check` — red flags only, exit 1 if any.
- `python3 graph/lib/view.py` — the full board, one screen.
