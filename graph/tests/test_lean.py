...
class Entry(unittest.TestCase):
    ...
    def test_its_own_open_pull_request_with_findings_is_revised(self):
        (self.ws.root / "contact").write_text("person@example.test\n")
        self.spec.write_text("---\nlean_status: pr_open\nlean_pr: https://example.test/pull/7\n---\n"
                             "Restyle the Log screen.\n")
        with mock.patch.object(lean_git, "unmerged", return_value=["origin/lean/log-screen"]), \
                mock.patch.object(lean_git, "threads", return_value="log.py:3\nFix it."), \
        ...
        self.assertEqual(("log.py:3\nFix it.", "https://example.test/pull/7"), feature.call_args.args[5:])

    def test_its_own_pull_request_with_nothing_to_fix_waits(self):
        (self.ws.root / "contact").write_text("person@example.test\n")
        self.spec.write_text("---\nlean_status: pr_open\nlean_pr: https://example.test/pull/7\n---\nx\n")
        with mock.patch.object(lean_git, "unmerged", return_value=["origin/lean/log-screen"]), \
                mock.patch.object(lean_git, "threads", return_value=""), \
                mock.patch.object(lean_run, "run_feature") as feature, \
                mock.patch.object(alert_email, "send", self.send):
            self.assertEqual(3, lean.main(self.argv()))
        feature.assert_not_called()
        self.assertIn("no unresolved review threads", self.mails[0][1])
