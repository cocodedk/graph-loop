...
class BaseBehindTest(unittest.TestCase):
    def test_a_base_behind_the_checkout_is_a_red_flag_naming_the_fix(self):
        line = base_behind(161, "campaign/graph")
        self.assertIn("!!", line)
        self.assertIn("161 commits behind", line)
        self.assertIn("git push . HEAD:campaign/graph", line)

    def test_a_base_at_the_checkout_is_no_flag(self):
        self.assertEqual("", base_behind(0, "campaign/graph"))

    def test_a_failed_measure_is_a_flag_not_a_green(self):
        line = base_behind(None, "campaign/graph", error="fatal: bad revision")
    ...
    def test_a_failed_measure_with_no_words_is_still_a_flag(self):
        self.assertIn("!! could not measure", base_behind(None, "campaign/graph"))

    def test_the_campaigns_recorded_branch_wins_over_the_environment(self):
        rows = [{"kind": "init", "branch": "campaign/other"}, {"kind": "claimed"}]
        self.assertEqual("campaign/other", recorded_branch(rows, "campaign/graph"))
        self.assertEqual("campaign/graph", recorded_branch([{"kind": "init", "branch": ""}], "campaign/graph"))
        self.assertEqual("campaign/graph", recorded_branch([], "campaign/graph"))
...
class BaseMeasureTest(unittest.TestCase):
    ...
    ABSENT = (128, "", "fatal: 'refs/heads/campaign/graph' - not a valid ref")
    ...
    def test_a_clean_count_is_the_plain_flag(self):
        line = base_measure("campaign/graph", self.runner((0, "abc\n", ""), (0, "3\n", "")))
        self.assertIn("3 commits behind", line)

    def test_a_silent_missing_branch_is_first_use(self):
        self.assertEqual("", base_measure("campaign/graph",
    ...
    def test_a_missing_branch_after_a_keep_is_a_flag(self):
        line = base_measure("campaign/graph", self.runner(self.ABSENT, self.NOT_THERE),
                            established=True)
        self.assertIn("!!", line)
    ...
    def test_a_probe_that_fails_with_words_is_a_flag(self):
        ...
        line = base_measure("campaign/graph", self.runner(broken, broken))
        self.assertIn("could not measure", line)
...
class RealGitTest(unittest.TestCase):
    ...
    `refs/heads/refs/heads/campaign/graph`, which is silently not there — so the
    ...
    def test_both_spellings_measure_the_branch_that_is_there(self):
        root = repo()
        subprocess.run(("git", "-C", root, "branch", "campaign/graph", "HEAD"),
                       capture_output=True, check=True)
        for base in ("refs/heads/campaign/graph", "campaign/graph"):
    ...
    def test_a_tag_of_that_name_does_not_stand_in_for_the_missing_branch(self):
        """`rev-parse` answers with `refs/tags/refs/heads/campaign/graph`, so
        ...
        root = repo()
        subprocess.run(("git", "-C", root, "tag", "refs/heads/campaign/graph", "HEAD"),
                       capture_output=True, check=True)
        line = base_measure("refs/heads/campaign/graph", self.git(root), established=True)
        self.assertIn("GONE", line)
        self.assertEqual("", base_measure("campaign/graph", self.git(root)))   # not yet cut
    ...
    def corrupt(self, content: str) -> str:
        ...
        subprocess.run(("git", "-C", root, "branch", "campaign/graph", "HEAD"),
                       capture_output=True, check=True)
        (pathlib.Path(root) / ".git/refs/heads/campaign/graph").write_text(content, "utf-8")
        return root
        ...
        for what, content in (("malformed", "not-a-sha\n"),
            ...
            for established in (False, True):
                with self.subTest(ref=what, established=established):
                    line = base_measure("campaign/graph", self.git(root),
    ...
    def test_a_repository_that_cannot_answer_is_a_failed_measure(self):
        ...
        line = base_measure("campaign/graph", self.git("/nonexistent"), established=True)
        self.assertIn("could not measure", line)
        self.assertNotIn("GONE", line)
