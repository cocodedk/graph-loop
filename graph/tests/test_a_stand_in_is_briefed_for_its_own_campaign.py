...
STAND-IN.md named `campaign/graph` and `scratchpad/graph-campaigns/current` in
...
GRAPH_CAMPAIGN and GRAPH_BRANCH set, so every tool the stand-in runs (the Stop
...
class BriefTest(HandoffRig):
    ...
    def test_the_brief_names_this_campaigns_branch_and_directory(self):
        self.assertIn("started", self.handoff(GRAPH_BRANCH="campaign/other").stdout)
        ...
        self.assertNotIn("campaign/graph", brief)
        self.assertNotIn("graph-campaigns/current", brief)

    def test_the_session_runs_with_its_own_campaign_and_branch_set(self):
        self.handoff(GRAPH_BRANCH="campaign/other")
        started = (self.state / self.session).read_text("utf-8")
        self.assertIn(f"GRAPH_CAMPAIGN='{self.camp}'", started)
        self.assertIn("GRAPH_BRANCH='campaign/other'", started)
    ...
    def test_a_campaign_whose_init_recorded_no_branch_gets_its_drivers(self):
        ...
        self.assertIn("started", self.handoff(GRAPH_BRANCH="").stdout)
        self.assertIn("git merge campaign/graph", (self.state / "brief").read_text("utf-8"))
