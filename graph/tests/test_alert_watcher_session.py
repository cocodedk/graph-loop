...
class HeartbeatProducerTest(unittest.TestCase):
    ...
    def run_heartbeat(self, config_dir: str, role: str = "", session_id: str = "",
        ...
        env = dict(os.environ, GRAPH_CAMPAIGN=str(self.camp), CLAUDE_CONFIG_DIR=config_dir)
        env.pop("WATCHER_ROLE", None)
        if role:
            env["WATCHER_ROLE"] = role
        env.pop("GRAPH_MESSENGER", None)
        if messenger:
            env["GRAPH_MESSENGER"] = "1"
        stdin = json.dumps({"session_id": session_id, "hook_event_name": "Stop"}) if session_id else ""
        return subprocess.run(["bash", str(GRAPH / "heartbeat.sh")], env=env, input=stdin,
...
class DryRunTest(unittest.TestCase):
    ...
    def dry(self, extra_env: dict | None = None) -> str:
        env = dict(os.environ, GRAPH_CAMPAIGN=str(self.camp), DRY="1",
                   PATH=f"{self.stub}:{os.environ['PATH']}")
        if extra_env:
            env.update(extra_env)
        return subprocess.run(["bash", str(GRAPH / "start-watcher.sh")], env=env,
    ...
    def test_any_argument_at_all_is_rejected(self):
        ...
        for args in (["--resume", "x"], ["-rabc"], ["--name", "x"]):
            with self.subTest(args=args):
                result = subprocess.run(["bash", str(GRAPH / "start-watcher.sh"), *args],
