...
`graph-goal.py run` was: campaign A's supervisor and driver could both be dead
...
THEIRS = [["8001", "0:05", "bash", "/x/supervisor.sh"],
          ["8002", "0:05", "python3", "/x/graph-goal.py", "run", "--idle-seconds", "300"]]
...
class DriverTest(Campaign):
    ...
    def board(self, our_driver: int | None) -> list[str]:
        ...
        with unittest.mock.patch.object(view, "Workspace", StubSpace), \
             ...
             unittest.mock.patch.object(view, "health", lambda: ["  8002 0:05 python3 /x/graph-goal.py run"]), \
