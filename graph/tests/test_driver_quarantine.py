...
_spec = importlib.util.spec_from_file_location("graph_goal_quarantine", HERE / "graph-goal.py")
...
def _run_spinning_on(status: str, needs: list[str] | None = None,
    ...
    with unittest.mock.patch.object(Keeper, "pending", lambda self: []), \
         ...
         unittest.mock.patch.object(graph_goal, "run_lanes", lambda *a, **k: (1, False)), \
         unittest.mock.patch("driver_turn.watchdog_check", lambda *a, **k: verdict):
        assert graph_goal.command_run(args) == 0
