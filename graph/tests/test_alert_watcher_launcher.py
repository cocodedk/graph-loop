...
class PrimaryLauncherTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.fake.write_text(
            ...
            "        sid = argv[i + 1]\n"
            "subprocess.run(['bash', " + repr(str(GRAPH / "heartbeat.sh")) + "],\n"
    ...
    def test_starting_the_primary_on_a_fresh_campaign_keeps_its_heartbeat_alive(self):
        result = subprocess.run(["bash", str(GRAPH / "start-watcher.sh")], env=self._env(),
    ...
    def test_an_existing_session_file_is_kept_not_overwritten(self):
        ...
        subprocess.run(["bash", str(GRAPH / "start-watcher.sh")], env=self._env(),
                       check=False, capture_output=True, text=True)
