...
class InitRig(unittest.TestCase):
    def init(self, root: str, branch: str) -> subprocess.CompletedProcess:
        """Run the real `graph-goal.py init` against this repository."""
        ...
        return subprocess.run(
            (sys.executable, str(GRAPH / "graph-goal.py"), "init",
             "--backlog", str(backlog), "--goal", "prove it"),
            env=dict(os.environ, GRAPH_REPO=str(root), GRAPH_CAMPAIGN=str(self.camp),
                     GRAPH_BRANCH=branch),
