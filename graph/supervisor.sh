...
# Usage: nohup bash graph/supervisor.sh > /dev/null 2>&1 &
set -u
HERE="$(cd "$(dirname "$0")" && pwd)"
# The campaign the DRIVER will use, asked of the loop itself: deriving it from
# this script's own path meant that with GRAPH_REPO set the supervisor watched
...
while true; do
  ...
  python3 "$HERE/graph-goal.py" plan >> "$W/run.log" 2>&1
  ...
  started=$(date +%s)
  python3 "$HERE/graph-goal.py" run --idle-seconds 300 >> "$W/run.log" 2>&1
  code=$?; ran=$(( $(date +%s) - started ))
  echo "$(date -Is) driver exited rc=$code after ${ran}s" >> "$W/supervisor.log"
  python3 "$HERE/graph-goal.py" report > "$W/reports/report-$(date +%Y%m%d-%H%M).txt" 2>&1
  ...
  if [ "$ran" -lt 30 ]; then
    ...
    if [ "$fast_failures" -ge 5 ]; then
      ...
      stand_down "the driver died five times in a row, so nothing is running. Read supervisor.log, fix what killed the driver, then start it again: bash graph/supervisor.sh" \
