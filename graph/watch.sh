...
#   bash graph/watch.sh          keep looking, every REFRESH_SECONDS
#   bash graph/watch.sh -1 [-v]   one look and stop
#   bash graph/watch.sh -v        keep looking, and show the last review
#   bash graph/watch.sh --check   only the red flags; exit 1 if there are any
#   bash graph/watch.sh --ledger  when, which card, what came of it
#   bash graph/watch.sh --read    mark the alerts shown now as read; later ones still show
#   Ctrl-C stops it.
REFRESH_SECONDS=15
