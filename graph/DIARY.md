...
**What it is.** `graph-goal.py` works a backlog of atomic tasks to done: it picks
...
6. **Nothing was kept.** Accepted work lived in a temporary worktree: a reboot
   ...
   `campaign/graph`, and each task starts from that tip.
...
**`graph-goal.py`** is the driver. Six verbs, and the one that matters most is
