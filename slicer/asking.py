...
def prompt(repo: pathlib.Path, sources: list[pathlib.Path], rows: list[dict],
    ...
    return (
        ...
        "  needs: []\n"
        "  note: File boundaries and rationale.\n"
        "  atoms:\n"
        "    - name: feature\n"
        "      stage: 1\n"
        ...
        "```\n"
        "An atom may also carry, only when it needs them: note, needs, uses, creates, "
        "gate_files_are_the_work, gate_until_kept, gate_when_kept, gate_has_side_effects, "
        "helper_verbs, may_add_files, expect_red.\n"
