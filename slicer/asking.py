...
def prompt(repo: pathlib.Path, sources: list[pathlib.Path], rows: list[dict],
    ...
    return (
        ...
        "Answer with YAML only. MOLECULE answer shape (replace example values):\n"
        "```yaml\n"
        "result: MOLECULE\n"
        "reason: An approved claim is not implemented yet.\n"
        "molecule:\n"
        "  name: implement-feature\n"
        "  source: [approved/source.md:1]\n"
        "  goal: Implement the approved feature.\n"
        "  why: The feature is missing.\n"
        "  needs: []\n"
        "  note: File boundaries and rationale. # optional\n"
        "  atoms:\n"
        "    - name: feature\n"
        "      stage: 1\n"
        "      goal: Implement the feature in its source file.\n"
        "      files: [src/feature.py]\n"
        "      gate: |\n"
        "        set -e -o pipefail\n"
        "        python -m unittest discover -s tests\n"
        "      done_when: The feature acceptance test passes.\n"
        "      note: This atom owns the feature source. # optional\n"
        "      needs: [] # optional\n"
        "      uses: [] # optional\n"
        "      creates: [src/feature.py:feature] # optional\n"
        "      gate_files_are_the_work: false # optional\n"
        "      gate_until_kept: false # optional\n"
        "      gate_when_kept: '' # optional\n"
        "      gate_has_side_effects: false # optional\n"
        "      helper_verbs: [] # optional\n"
        "      may_add_files: true # optional\n"
        "      expect_red: '' # optional\n"
        "```\n"
        "result is MOLECULE, NO_GAP or NEEDS_PERSON. source holds anchors, each an approved "
        "source path spelled exactly as listed above followed by "
        f"ONE existing line number, like `{anchor}` — never a range "
        "like :13-17, never a whole file. Its name is new and "
        ...
        "runnable and also requires files, gate and done_when. "
        "stage is a positive integer such as 1, never a word. If an atom's gate runs one of its own "
